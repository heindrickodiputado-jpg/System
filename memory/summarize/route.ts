...
export async function POST(req: NextRequest) {
  ...
  const convoText = messagesResult.rows
    .map((r) => `${String(r.role).toUpperCase()}: ${r.content}`)
    .join('\n\n');
  ...
  try {
    ...
    if (summary === 'NOTHING_NEW' || !summary) {
      return NextResponse.json({ skipped: true, reason: 'Nothing new' });
    }
    ...
    if (!bullets.length) {
      return NextResponse.json({ skipped: true, reason: 'No bullets parsed' });
    }

    // Fetch existing auto memories for dedup
    const existingResult = await db.execute(
      "SELECT text FROM memories WHERE type = 'auto'"
    );
    ...
    // Trim to 80 auto memories max
    const countResult = await db.execute(
      "SELECT COUNT(*) as count FROM memories WHERE type = 'auto'"
    );
    const count = countResult.rows[0]?.count as number;

    if (count > 80) {
      const excess = count - 80;
      await db.execute({
        sql: "DELETE FROM memories WHERE type = 'auto' AND id IN (SELECT id FROM memories WHERE type = 'auto' ORDER BY created_at ASC LIMIT ?)",
        args: [excess],
      });
    }
